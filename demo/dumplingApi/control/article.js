...
const FoodModel= require('../db/model/articleadd')
...
async function get(page,pageSize){
  ...
  return  {foods,allCount,page,pageSize}
}
...
async function getByKw(kw,page,pageSize){
 ...
 let  allFoods=await FoodModel.find({$or:[{title:{$regex:regex}}]})
 let  allCount = allFoods.length
 let  foods=await FoodModel.find({$or:[{title:{$regex:regex}}]}).skip((page-1)*pageSize).limit(pageSize)
 console.log(regex,allCount,allFoods,foods,kw,pageSize,page)
 return {foods,allCount}
}
